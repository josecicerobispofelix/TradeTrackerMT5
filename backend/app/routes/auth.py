...
def _send_reset_email(to_email: str, token: str) -> None:
    ...
    msg = EmailMessage()
    msg["Subject"] = "RedefiniÃ§Ã£o de senha - TradeTrackerMT5"
    msg["From"] = sender
    msg["To"] = to_email
    msg.set_content(
        "Use o cÃ³digo abaixo para redefinir sua senha no TradeTrackerMT5:\n\n"
        f"{token}\n\n"
...
async def register(
    ...
    # Se for o primeiro usuÃ¡rio, vincula dados existentes sem usuÃ¡rio
...
async def request_reset(
    ...
    user = await session.scalar(select(User).where(User.email == email))
    if not user:
        return {"message": "Se o e-mail existir, enviaremos as instruÃ§Ãµes."}
    ...
    return {
        "message": "Reset gerado. Configure SMTP para envio automÃ¡tico.",
