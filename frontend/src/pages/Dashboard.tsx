  ...
  const handleOpenPdfFolder = async () => {
    ...
    if (api?.open_path) {
      try {
        ...
        if (!result?.success) {
          throw new Error(result?.error || "No foi possvel abrir a pasta do PDF.");
        }
        return;
      } catch (err) {
        setDarfStatus((err as Error).message);
      ...
      if (api?.save_pdf_prompt) {
        try {
          ...
          if (response?.success) {
            ...
            setDarfStatus(null); // evita duplicar com "ltimo PDF"
            return;
          }
        ...
        <div className="dashboard-main">
          ...
          <div className="cards kpi-grid">
            <div
              ...
              title="Resultado líquido = lucro + comissão + swap.  o valor final real do período."
            >
              ...
              {darfResult ? (
                <div className="helper">
                  {(() => {
                    ...
                    const resumo = `Fechamento ${monthLabel}: ${isProfit ? "lucro" : "prejuzo"} de ${brl} (${usd}).`;
                    const aliquota = `Alquota ${(darfResult.tax_rate * 100).toFixed(2)}%.`;
                    const imposto =
                      darfResult.tax_due > 0
                        ? `Imposto devido: ${formatCurrency(darfResult.tax_due, "BRL")}.`
                        : "Imposto devido: R$ 0,00. Nada a pagar neste ms.";

                    let extraMessage = (darfResult.message || "").trim();
                    const extraLower = extraMessage.toLowerCase();
                    const mentionsNoTax = extraLower.includes("no h imposto a pagar");
